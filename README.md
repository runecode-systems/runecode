...
Non-Nix fallback (e.g., Windows): install Go 1.25.x, Node `>=22.22.1 <25` with npm, `just`, and either a `tlc` binary or Java 17+ plus `tla2tools.jar` (or set `TLA2TOOLS_JAR`). CI provisions that jar from the flake-pinned Nix package cache, verifies its SHA-256 against the Nix build output, then run:
