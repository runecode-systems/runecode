...
- CI workflow security is explicit: least-privilege token permissions and full-SHA pinning for all `uses:` actions.
- CI validates `flake.lock` metadata before executing flake checks (`nix flake lock --no-update-lock-file`) and verifies it remains unchanged at job end.
...
- Dev-shell discoverability output (`Entering dev shell`, `just --list`) is interactive-only so command-mode `nix develop -c ...` output stays clean.
- High-leverage files (`flake.nix`, `flake.lock`, `.envrc`, `justfile`, `.github/CODEOWNERS`, `.github/workflows/*`) are protected via CODEOWNERS.
