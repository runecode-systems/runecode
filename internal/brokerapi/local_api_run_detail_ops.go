...
func (s *Service) runDetail(runID string) (RunDetail, bool, error) {
	...
	runtimeEvidence := s.RuntimeEvidence(runID)
	...
	return buildRunDetail(summary, verification, artifactsForRun, classCount, pendingIDs, policyRefs, approvals, runtimeFacts, runtimeEvidence, runnerAdvisory, currentInstanceID), true, nil
}
...
func buildRunDetail(summary RunSummary, verification AuditVerificationSurface, artifactsForRun []artifacts.ArtifactRecord, classCount map[string]int, pendingIDs []string, policyRefs []string, approvals []ApprovalSummary, runtimeFacts launcherbackend.RuntimeFactsSnapshot, runtimeEvidence launcherbackend.RuntimeEvidenceSnapshot, runnerAdvisory artifacts.RunnerAdvisoryState, currentInstanceID string) RunDetail {
	...
	authoritativeState := buildAuthoritativeRunState(summary, artifactsForRun, pendingIDs, manifestHashes, policyRefs, approvals, runtimeFacts, runtimeEvidence, currentInstanceID)
