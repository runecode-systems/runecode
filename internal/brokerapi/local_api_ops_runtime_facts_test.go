...
func TestRunDetailRuntimeFactsProjectionSurvivesServiceRestart(t *testing.T) {
	...
	preRestartEvidence := svc.RuntimeEvidence("run-runtime-restart")
	...
	if runGet.Run.AuthoritativeState["attestation_evidence_digest"] != preRestartEvidence.Attestation.EvidenceDigest {
		t.Fatalf("authoritative_state.attestation_evidence_digest = %v, want %q after restart", runGet.Run.AuthoritativeState["attestation_evidence_digest"], preRestartEvidence.Attestation.EvidenceDigest)
	}
	if runGet.Run.AuthoritativeState["attestation_verification_digest"] != preRestartEvidence.AttestationVerification.VerificationDigest {
		t.Fatalf("authoritative_state.attestation_verification_digest = %v, want %q after restart", runGet.Run.AuthoritativeState["attestation_verification_digest"], preRestartEvidence.AttestationVerification.VerificationDigest)
	}
}
...
func TestRunSummaryAndDetailProjectRecordedLauncherRuntimeFacts(t *testing.T) {
	...
	evidence := s.RuntimeEvidence("run-launcher-facts")
	if evidence.Attestation == nil || evidence.AttestationVerification == nil {
		t.Fatalf("runtime evidence attestation/verification missing: %#v", evidence)
	}
	...
	assertRuntimeFactsAttestationReferenceProjection(t, state, evidence)
...
func assertRuntimeFactsAttestationReferenceProjection(t *testing.T, state map[string]any, evidence launcherbackend.RuntimeEvidenceSnapshot) {
	t.Helper()
	if state["attestation_evidence_digest"] != evidence.Attestation.EvidenceDigest {
		t.Fatalf("authoritative_state.attestation_evidence_digest = %v, want %q", state["attestation_evidence_digest"], evidence.Attestation.EvidenceDigest)
	}
	...
	if state["attestation_verification_digest"] != evidence.AttestationVerification.VerificationDigest {
		t.Fatalf("authoritative_state.attestation_verification_digest = %v, want %q", state["attestation_verification_digest"], evidence.AttestationVerification.VerificationDigest)
	}
