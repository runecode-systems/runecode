...
- Keep verification reports explicit about verifier identity and trust-root identities; do not emit unsigned or ambiguous verifier-footprint summaries that hide which verifier implementation or trust roots produced a report
- Distinguish invalid present evidence from missing required evidence in verifier findings; export completeness gaps, missing approval linkage, and limited negative-capability support should surface as first-class report outcomes rather than being folded into generic failure text
- Derive operator-visible anchoring posture from the verified evidence set and report findings, including explicit degraded states for missing or deferred external anchoring, rather than from ad-hoc UI status inference
- When verification reasons depend on meta-audit evidence such as bundle export, restore, verifier configuration, or trust-root updates, evaluate those receipts inside the verified scope rather than from unrelated historical records
