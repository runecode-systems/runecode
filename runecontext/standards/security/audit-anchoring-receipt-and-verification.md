...
- For external anchoring, keep the signed `AuditReceipt` envelope authoritative while storing target-specific proof bytes, provider receipts, transcripts, and similar bulky target artifacts as separately digest-addressed sidecars referenced by the receipt or bound evidence payload; do not make target-native proof blobs a parallel authoritative receipt family
...
- Keep external anchoring bound to the original signed seal digest; successful anchoring may add durable external evidence and sidecar references, but must not rewrite, rehash, or supersede the original `AuditSegmentSeal` as the verification subject
