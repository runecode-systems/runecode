...
This change now explicitly reuses the verified project-substrate binding model, the shared audit-evidence authority model, and the shared trusted-policy boundary so the first ZK proof lane does not introduce a second project-truth surface, a second audit authority, or a second authorization engine.
