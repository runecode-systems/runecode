...
- Treat the remaining TUI presentation work as a beta-blocking product gate rather than optional visual cleanup: the current TUI has strong Bubble Tea, Bubbles, Lip Gloss, route, inspector, command, and broker-state foundations, but still needs a calmer shell, professional modal overlays, stricter rendered/structured/raw separation, route-by-route product-language cleanup, display-width-safe styling, and captured visual walkthrough evidence before it can be called polished.
...
- TUI polish means more than using Bubble Tea and Lip Gloss primitives. The beta bar requires a professional operator experience: calm hierarchy, product language in primary panes, debug detail moved to structured/raw modes, clear state cards, command discovery that feels intentional, responsive narrow/wide layouts, and honest broker-owned progress without synthetic optimism.
