...
- [ ] Preserve the same required-attestation supported posture used on other platforms and do not allow macOS-specific TOFU fallback or override behavior.

## Optional Virtualization.framework Backend

- [ ] Evaluate adopting Virtualization.framework for improved UX and performance.
- [ ] Keep the capability model, runtime-image identity, launch/session/attachment semantics, launch-evidence semantics, attestation evidence and verification semantics, and audit semantics unchanged.
...
- [ ] Ensure missing macOS prerequisites or missing attestation capability fail closed rather than downgrading to a supported TOFU posture.
...
- [ ] macOS microVM launch consumes the same signed immutable runtime assets, preserves the same launch-admission and launch-evidence semantics used on other platforms, and requires the same valid attestation posture for supported production and user-facing runtime use.
...
- [ ] No macOS-specific supported path permits TOFU trust decisions.
