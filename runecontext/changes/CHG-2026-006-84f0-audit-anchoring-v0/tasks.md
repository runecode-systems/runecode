...
- [ ] Define a typed anchor receipt object stored as a first-class artifact:
  ...
  - includes: `{schema_id, schema_version, anchor_kind, audit_segment_id, audit_segment_root_hash, created_at, signer_key_id, signer_scope, key_protection_posture, presence_mode, approval_assurance_level, approval_decision_hash, receipt_payload}`
...
- [ ] Implement an MVP anchoring mode that requires explicit user presence to mint a receipt:
  - sign the segment root commitment using the purpose-scoped `audit_anchor` authority with a user-assurance gate (see `runecontext/changes/CHG-2026-005-cfd0-crypto-key-management-v0/`)
