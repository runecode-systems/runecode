...
- Product-shipped built-in workflow identities from `CHG-2026-049-1d4e-first-party-runecontext-workflow-pack-v0` remain non-overridable; later custom workflow adoption must use an explicit separate registration/catalog path rather than repository-local shadowing.
...
- Where authoring targets draft-like workflows, it must preserve the CHG-049 artifact-first plus explicit promote/apply posture rather than collapse draft generation and canonical mutation into one implicit ambient local-edit path.
...
- Authoring UX may prepare custom workflow content only through explicit separate registration/catalog flows; it must not replace or shadow product-shipped built-in workflow IDs.
...
- Custom Workflow Registration/Catalog Path
