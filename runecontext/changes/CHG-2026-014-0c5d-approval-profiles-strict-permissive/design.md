...
- When `CHG-2026-059-7b31-cross-machine-evidence-replication-restore-v0` is active, profiles must not weaken the publication durability barrier for `git_remote_ops` or any equivalent publication-sensitive remote-state-mutation lane; exact-action approval remains necessary but is not sufficient when healthy evidence durability preconditions are unsatisfied.
...
- When the replication and restore lane is active, publication-sensitive remote-mutation approval payloads should also remain bound to the required evidence checkpoint digest or equivalent durable pre-publication evidence boundary defined by that lane.
