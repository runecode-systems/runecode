...
- [ ] Introduce one trusted authoritative executor registry shared across policy and runner dispatch.
- [ ] Define the read-only runner projection rules for that registry so the runner does not invent executor semantics locally.
...
- [ ] Bind workspace execution to `RunPlan` executor entries rather than ad hoc workflow-local executor choice.
...
- [ ] Executor semantics stay authoritative in trusted Go and do not drift into an independent runner-local policy surface.
