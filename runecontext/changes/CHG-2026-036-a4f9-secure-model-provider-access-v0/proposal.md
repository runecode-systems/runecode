## Summary
Track secure model and provider integration as one project-level plan covering shared secret lifecycle, canonical model egress boundaries, auth flows, bridge contracts, provider-specific feature lanes, and the cross-feature contracts they must all inherit.
...
Now that the `secretsd` and `model-gateway` foundation has been refined, the provider umbrella also needs to state those shared contracts explicitly. Otherwise later provider lanes could still drift on short-lived token handoff, canonical request boundaries, destination identity, operator posture, or quota semantics even while claiming to preserve the same high-level trust model.
...
This umbrella should now also record the newly reviewed foundation so later provider-specific changes inherit it explicitly instead of quietly narrowing or redefining it.
...
## Impact
Creates a project-level anchor for provider and model-access sequencing while leaving delivery details in feature changes and making the inherited shared security contracts explicit for all later provider work.
