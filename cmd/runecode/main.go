...
func startRepoBrokerProcess(scope localbootstrap.RepoScope) error {
	...
	cmd := exec.Command(binary,
		"--state-root", scope.StateRoot,
		"--audit-ledger-root", scope.AuditLedgerRoot,
		"serve-local",
		"--runtime-dir", scope.LocalRuntimeDir,
		"--socket-name", scope.LocalSocketName,
	)
	cmd.Dir = scope.RepositoryRoot
	cleanupIO, err := prepareDetachedBrokerProcessIO(cmd, scope.LocalRuntimeDir)
	if err != nil {
		return err
	}
	defer cleanupIO()
